...
Каталог сервисов и внешних интеграций TT3Dato.
...
2. Интеграция фиксируется в `docs/repositories/stack-mappings.json` и `docs/ATTRIBUTIONS.md`.
...
| `telegram` | CANDIDATE | Funnel-канал в TT3Dato: бот @HuckleberryFinn18Bot (webhook/Vercel) пишет сообщения владельца в `apps/telegram-client/inbox/` и файлы в `Don'tReadMe/`, без LLM; безопасность по Aegis (токен из env + chat_id allowlist). Ветка `260802-sec-owner-allowlist` MERGED. |
| `telegram-client` | CANDIDATE | Клиентский бот @Audit_TT3dato_bot: сбор информации о компании (12–18 вопросов, кнопки/текст/голос) в `docs/clients/<slug>/`. Ветка `260802-feat-client-bot` ждёт merge. |
