...
- Правила (обязательные стандарты) живут в `agents/skills/rules/` и всегда загружены.
- Навыки не добавляются без регистрации в `docs/repositories/inventory.json`.
- Использование навыка не расширяет права агента — права определяет `docs/agents/permissions.md`.
